...
class SignDigitDataset(Dataset):
    def __init__(self, root_dir='data/', h5_name='train_signs.h5',
        ...
        self.dataset_labels = np.array(
            self._read_data(self.h5_path)[key + '_y'])
    ...
    def __getitem__(self, index):
        ...
        #####################################################################################
        self.transform = get_transformations(self.dataset_images[index].size)
        img = self.transform(self.dataset_images[index])
        label = get_one_hot(label=self.dataset_labels[index], num_classes=6)
        #####################################################################################
        #                                 END OF YOUR CODE                                  #
        #####################################################################################
        return {
